...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.22\linewidth} p{0.23\linewidth}
    ...
    Executed evidence
      ...
      & $37$ latest-successful leaves
      & $48$ latest-successful leaves \\
    Technology coverage
...
$2700$ future leaves, for $2800$ leaves in total. The current thesis evidence consists of $85$ distinct latest-successful scenario leaves: $37$ leaves from the main \texttt{scenario\_tree} registry and $48$ leaves from the \texttt{scenario\_tree\_output34} registry. The main registry covers the baseline current-stock case and frozen-stock future controls across all three RCP pathways and all future climate windows. The \texttt{output34} registry adds selected design-year stochastic cohort runs, including moderate and high electrification technology cases.
...
%     \texttt{scenario\_tree\_output34}
%       & targeted design-year subset & $48$
%       & peak, diversity, and technology-stress comparison \\
%     \midrule
...
This section reports the executed subset of the scenario tree. Climate-pressure and annual heating-change results draw on the 37 successful full-window leaves (daily cadence); peak-load and grid-stress results draw on the 48 successful design-year leaves in \texttt{scenario\_tree\_output34}, which run hourly 30-household stochastic cohorts. Neither ledger includes active cooling final energy, so cooling is treated as a demand-pressure indicator only.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}l l r r r r r@{}}
    ...
    \midrule
    \texttt{current\_stock} & baseline / historical & 3667 & 18870 & 17.53 & +0.0 & 143 \\
    \texttt{frozen\_stock} & 2080--2100 / RCP 8.5 & 3549 & 13433 & 17.54 & +0.1 & 115 \\
    \texttt{moderate\_electrification} & 2080--2100 / RCP 8.5 & 13164 & 7939 & 87.00 & +396.4 & 8700 \\
    \texttt{high\_electrification\_pv\_ev} & 2080--2100 / RCP 8.5 & 33593 & 1837 & 184.15 & +950.7 & 8760 \\
    \bottomrule
  \end{tabular}}
\end{table}

\subsubsection*{Peak grid import}
Across the executed design-year subset, technology choice dominates climate forcing for peak grid import. Under frozen stock, long-term RCP\,8.5 leaves the cohort peak almost unchanged ($17.53$ to $17.54$ kW), whereas switching the technology case under the same forcing lifts it to $87.00$ kW under moderate electrification and $184.15$ kW under high electrification with PV and EV.
...
In the cold-design-year long-term RCP\,8.5 case, the dynamic-tariff bill falls from about € 3345 per household in the historical current-stock baseline to € 2762 under frozen stock, because gas demand falls with heating pressure. The same tariff raises bills to about € 5172 under moderate electrification and € 10741 under the high-electrification PV--EV case. Across the executed tariff scenarios, electrification remains more expensive under the current tariff, export-credit and capex assumptions; the current outputs therefore should not be framed as showing favourable electrification economics.

\subsubsection*{Emissions}
Operational emissions use a fixed electricity-emission factor. Under that assumption, frozen stock falls to about $3246$ kgCO$_2$/household/year ($-25.6\,\%$), moderate electrification falls to about $3578$ kgCO$_2$/household/year ($-18.0\,\%$), and the high-electrification PV--EV case rises to about $5410$ kgCO$_2$/household/year ($+24.0\,\%$). This result is sensitive to the fixed electricity factor and should be interpreted as a scenario-accounting result, not as a general decarbonisation conclusion.


\subsection{Sensitivity and Uncertainty}
\label{sec:sensitivityStochasticRobustness}

The executed scenario results contain three stochastic seeds per design-year scenario, and the comparison files report P10/P50/P90 bands across those seeds.
