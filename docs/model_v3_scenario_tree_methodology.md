...
The current audit should also be used to screen result claims before presentation. In particular, the annual useful-heating climate-only comparison is audit-flagged when future `tech_frozen_stock` rows show lower HDD but higher useful-heating demand, especially if compared rows mix different Git commits, dirty-tree states, or Belgian technology-input hashes. Those rows are still useful for debugging traceability, but they should not be treated as final thesis results until the relevant baseline and future frozen-stock leaves are rerun from one frozen clean revision and summaries/comparisons are regenerated.
