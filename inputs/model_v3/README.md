...
These files are stored as real files/directories inside `inputs/model_v3/`, not symlinks.

## Active Runtime Sources

- `building/archetype_parameters_merged_v2.csv`
  Authoritative `model_v3` archetype table for stock identity, geometry, heat loss, thermal mass, comfort defaults, solar defaults, and airflow defaults.
- `building/envelope_archetypes_v1.csv`
  Reproducible envelope-area reconstruction behind runtime `UA_W_per_K` and `H_W_per_K` values.
- `weather/aws_1hour_Uccle.csv`
...
- The merged archetype table is the active source of truth for `UA`, `C`, volume, comfort defaults, solar parameters, and airflow defaults.
- Runtime `UA`/`H` values are generated from `building/envelope_archetypes_v1.csv` using `python3 -m model_v3.building.rebuild_archetype_envelope --repo-root . --print-summary`.
...
Top-level `model_v3` runs persist runtime artifacts under `outputs/model_v3/`.
