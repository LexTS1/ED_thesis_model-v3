...
def _run_config(tmp_path: Path, *, leaf_id: str, technology_case_id: str, year: int) -> dict:
    ...
    return {
        ...
        "technology": {
            ...
            "metadata_file": str(REPO_ROOT / "config" / "model_v3" / "scenario_tree" / "technology_cases.yaml"),
            "belgian_technology_inputs": str(REPO_ROOT / "config" / "model_v3" / "belgian_technology_inputs.yaml"),
        },
        "stochastic": {
            "realization_id": realization_id,
            "seed_value": 0,
...
def test_high_electrification_pv_ev_scenario_maps_to_ev_outputs(tmp_path: Path) -> None:
    ...
    assert model_config["der"]["pv"]["enabled"] is True
    assert model_config["mobility"]["ev"]["enabled"] is True

    results = run_annual_simulation(config=model_config)

    assert results["annual_ev_charging_kWh"] > 0.0
    assert results["annual_pv_generation_kWh"] > 0.0
    assert results["profile_frame"]["P_el_ev_charging_W"].max() > 0.0
    assert results["profile_frame"]["P_pv_generation_W"].max() > 0.0
